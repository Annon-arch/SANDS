...
# Pro-BJP (1), Anti-BJP (-1), Pro-Congress (2), Anti-Congress (-2), Pro-AAP (3), Anti-AAP (-3), and, Other (0).
# MappingIndia["-1"] = 0
# MappingIndia["1"] = 1
# MappingIndia["-2"] = 2
# MappingIndia["2"] = 3
...
LABEL_MAPPING_INV = {6:'NONE', 0:'AGAINST BJP', 1:'FOR BJP',
                     2:'AGAINST CONGRESS', 3:'FOR CONGRESS',
                     4:'AGAINST AAP', 5:'FOR AAP'}
...
with open("NEW_VOCAB_India", "rb") as F:
    VOCAB = pickle.load(F)
    
with open("NEW_HASHTAGS_India", "rb") as F:
    HASHTAGS = pickle.load(F)

COMBINED_VOCAB = HASHTAGS.union(VOCAB)
print(len(COMBINED_VOCAB))

with open("NOT_ANNOTATED_IN_BATCHES7_India", "rb") as F:
    LEFT = pickle.load(F)

with open("EncodedDataFrameWithLabel_India", "rb") as F:
    EncodedDataFrameWithLabel = pickle.load(F)    

TweetInfoDF = pd.read_csv('TweetInfoDF_India.csv')

EncodedDataFrameWithLabel['Tweet'] = list(TweetInfoDF['text'])
EncodedTweetDFLabel_w_label = EncodedDataFrameWithLabel[EncodedDataFrameWithLabel['Tag']!=-1]
...
with open("train_test_India", "rb") as F:
...
for i,j in X_train.iterrows():
    ...
    Train_Target_list.append("Indian Political Parties")
    Train_Stance_list.append(target)
    Train_Id.append(i)
    Tweet_list.append(tw)
    Target_list.append("Indian Political Parties")
...
for i,j in X_val.iterrows():
    ...
    Test_Target_list.append("Indian Political Parties")
...
w2vmodel = word2vec.Word2Vec.load("Word2VecModel_India")
...
transformed_labels = transform_labels(labels, 7)
...
transformed_labels_test = transform_labels(labels_test, 7)
...
outfile = "india_results_quicktest_" + testsetting + "_" + modeltype + "_" + str(hidden_size) + "_" + dropout + "_" + tanhOrSoftmax + "_" + str(max_epochs) + "_" + testid + ".txt"
...
def train(testsetting, w2vmodel, tweets, targets, labels, ids,
    ...
    outfolder = "_".join([testid, modeltype, testsetting, "hidden-" + str(hidden_size), tanhOrSoftmax, "India"])
    ...
    with tf.Session() as sess:
        summary_writer = tf.summary.FileWriter("./out_India/save", graph_def=sess.graph_def)
